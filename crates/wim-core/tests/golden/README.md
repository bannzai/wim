...
コマンドライン (`:w` `:q` 等) と検索 (`/` `?` `n` `N`) は未実装 (issue #6)。マクロ記録・再生 (`q` `@`) も未実装 (issue #7)。これらのケースは各 issue の実装時にここへ追加する。

レジスタの中身・モード・undo 履歴の深さのように、バッファのテキストとカーソル位置に現れない状態はこの形式では検証できない。それらはコア側のユニットテスト (`src/editor.rs` 等) で見る。
